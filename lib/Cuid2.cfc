...
component
	...
	*/
	private string function generateHashBlock() {
		...
		var parts = createObject( "java", "java.io.ByteArrayOutputStream" ).init();

		parts.write( charsetDecode( getTickCount(), "utf-8" ) );
		parts.write( generateBytes( cuidLength ) );
		parts.write( charsetDecode( counter.getAndIncrement(), "utf-8" ) );
		parts.write( charsetDecode( processFingerprint, "utf-8" ) );

		var hex = hash( parts.toByteArray(), "sha-256" );
		var bytes = binaryDecode( hex, "hex" );
		...
		var parts = [
			charsetDecode( getTickCount(), "utf-8" ),
			generateBytes( cuidLength ),
			charsetDecode( counter.getAndIncrement(), "utf-8" ),
			charsetDecode( processFingerprint, "utf-8" )
		];

		var hashInputs = parts.reduce(
			( reduction, part ) => {
		...
		var hex = hash( javaCast( "byte[]", hashInputs ), "sha-256" );
		var bytes = binaryDecode( hex, "hex" );
